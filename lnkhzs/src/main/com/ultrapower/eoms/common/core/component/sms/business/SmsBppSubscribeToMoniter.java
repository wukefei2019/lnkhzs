 ...
 *
 */
public class SmsBppSubscribeToMoniter implements SmsSubscribeToMoniterService
{
	...
	private Logger log = LoggerFactory.getLogger(SmsBppSubscribeToMoniter.class);
	QueryAdapter query=new QueryAdapter();
	QueryAdapter bppQuery=new QueryAdapter("bpp");
	DataAdapter dataAdapter=new DataAdapter("bpp");
	...
	public void call()
	{
		...
		try{
			...
			RQuery query=new RQuery("SQL_SM_SmsBppSubscribeToMoniter.person",null);
			DataTable dt=query.getDataTable();
			log.info("SmsBppSubscribeToMoniter::call>> sms_monitor_person: "+dt.length()+" 条.");
			this.subscribe(dt);
			
			query=new RQuery("SQL_SM_SmsBppSubscribeToMoniter.group",null);
			dt=query.getDataTable();
			log.info("SmsBppSubscribeToMoniter::call>> sms_monitor_group: "+dt.length()+" 条.");
			this.subscribe(dt);

			query=new RQuery("SQL_SM_SmsBppSubscribeToMoniter.role",null);
			dt=query.getDataTable();
			log.info("SmsBppSubscribeToMoniter::call>> sms_monitor_role: "+dt.length()+" 条.");
			this.subscribe(dt);
		...
		for(int i=0;i<dtLen;i++)
		{
			...
			blntime=false;
			 ...
			 if ("1".equals(isdw)){//派代维
		        if ("WF4_EL_TTM_TTH".equals(baseschema)){
		          if (!"".equals(content) && !"".equals(processname)){
		            if ("移动二级处理中".equals(processname)){
		              isDirect = true;
		            }
		          }
		        }else{
		      ...
		      if ((isDirect) || ((blnitem) && (blntime))){
		    	...
		    	log.info("SmsBppSubscribeToMoniter::subscribe>> goal= "+goal+ " , content = "+content);
		        if (opflag){
	...
	private boolean checkBaseitem(String loginname,String baseid,String baseschema)
	{
		...
		if(baseitems!=null) 
		{
			String tblName="BS_F_"+baseschema;
			StringBuffer str=new StringBuffer();
			str.append("select baseitems from ");
			str.append(tblName);
			str.append(" where ");
			str.append(" baseId= ? ");
			try{
				DataTable dt=bppQuery.executeQuery(str.toString(), new Object[]{baseid});
				...
				System.out.println("checkBaseitem >> 当前工单不存在网络分类 Schema："+baseschema);
				return false;
			}
		}
		return true;
	...
	public HashMap<String,String> getDoActionPara(String relateId){
		...
		DataTable dt=bppQuery.executeQuery(p_sql, new Object[]{relateId});
		DataRow dr = null;

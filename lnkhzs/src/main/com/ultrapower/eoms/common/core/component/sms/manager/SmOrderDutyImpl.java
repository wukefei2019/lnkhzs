...
public class SmOrderDutyImpl implements SmOrderDutyService{
	...
	public void insertSmsMonitor() {
		try {
			...
			dt = query.executeQuery("select distinct t.*, u.loginname, u.mobile from bs_t_task_note t, bs_t_sm_user u, bs_t_sm_smsorderduty s " +
					"where t.userid = u.pid and u.loginname = s.loginname and t.informtype = 4  and t.active = 1 and u.status = 1 " +
					"and t.bookingtype = ? and t.state = ?", 3, 2);
			if (dt != null && dt.length() > 0) {
	...
	private void subscribe(DataTable dt, boolean isHoliday) throws Exception {
		...
		for (int i = 0; i < dt.length(); i++) {
			...
			// 针对该人员去匹配他的发送时间
			pdt = query.executeQuery("select * from bs_t_sm_smsorderduty t where t.loginname = ?" +
				...
				for (int j = 0; j < pdt.length(); j++) {
					...
					if (isHoliday && pdr.getLong("isholiday") == 1L) {// 如果是节假日，并且节假日不发短信，则跳过本次循环,否则继续下一步
						...
						if (noSend) {// 如果本条短信没有发送过，则继续下一步，如果已经发送过，则跳出循环
							opflag = insidesm.sendsm(goal, content, "duty", "", 0);
							if (opflag) {

...
<ubf:PanelGroup name="PG_Main" cell="4" type="tab" visiable="1" showtitle="1">
	<ubf:Panel name="Txt_ContentTitle" label="工单信息" visiable="1">
		...
		<ubf:SelectField name="BaseCountry" label="区县" type="sysdic" resource="BaseCity" paras="@!{BaseCity}" cell="1" visiable="1"   />
		<ubf:CharacterField name="remarks" label="备注" row="3" cell="4" visiable="1" length="3000" />
		<ubf:ViewField name="billList" label="电费信息表" row="5" cell="4" type="frame" visiable="1" url="${ctx}/omcs/calculate/elec_bill/wfDetail.jsp?pid=${displayCxt.fieldMap.bill_code}"/>
		<ubf:HiddenField name="bill_code" label="电费ID" cell="1" length="500" />
		<ubf:TimeField name="BaseDealOutTime" label="处理时限" row="1" cell="1" visiable="0" format="yyyy-MM-dd HH:mm:ss"  />
		...
		<ubf:AttachmentField name="AttachmentsList" label="附件列表" types="*.*" visiable="1">
			<atta:fileupload id="AttachmentsList_tag"
		      ...
		      displayAllAtt="self" baseSchema="WF4_EL_EF_AUDIT" baseID="${AttachmentsList_baseID}" 
		      templateID=""

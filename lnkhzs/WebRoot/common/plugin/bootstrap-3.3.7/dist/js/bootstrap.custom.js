...
//                    	}
    ...
    BootstrapTable.prototype.hover = function (index) {
		if(index<0){
			return;
		}
        ...
        for(var i=0; i<this.data.length;i++){
        	if(serialno==this.data[i].SERIALNO){
        		$(this.$body.children()[i]).addClass( "ts-bg-green" );
        	}else{
        		$(this.$body.children()[i]).removeClass( "ts-bg-green" );
        	}
        }
    };

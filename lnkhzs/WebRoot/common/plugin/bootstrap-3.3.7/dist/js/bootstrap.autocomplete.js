...
var autocomplete_get_opts = function (opts){
    opts = $.extend({
        id : new Date().getTime(),
        url : $ctx + "/omcs/baseAction/ajaxRQuery.action",
        relation : {},
        effect:{},
        limit:15,
        auto_before:function(){return true}
    }, opts);
    function getParams(query){
        var result = {
                matchCount : opts.limit,
                limit : opts.limit,
                sqlname : opts.sqlname,
        };
        // result[opts.matchInfo] = query;
        for ( var key in opts.relation) {
            var $e = opts.relation[key];
            if(typeof $e == 'string'){
                result["param." + key] = $e; 
            }else if($e.is(".ui-select")){
                result["param." + key] = $e.ui_select().val(); 
            }else {
                result["param." + key] = $e.val(); 
            }
        }
        return result;
    }
    return $.extend({
        source:function(query,process){
            ...
            this.$element.attr("cus-event",'source');
            $.post(opts.url, getParams(query),function(respData){
                    return process(respData);
            });
        },
        formatItem: opts.formatItem || function(item){
            var html = [];
            for (var i = 0; i < opts.columns.length; i++) {
                html.push("<span>"+item[opts.columns[i]] + "</span>")
            }
            return html.join("");
        },
        setValue:function(item){
//            var result = {};
//            for (var i = 0; i < opts.columns.length; i++) {
//                result[opts.columns[i]] = item[opts.columns[i]];
//            }
//            return result;
            return item;
        },
        ...
        select: function () {
            this.$element.attr("cus-event",'select');
            var $el = this.$menu.find('.active');
            this.$element.val(this.updater($el.attr(opts.matchInfo))).change();
            for ( var key in opts.effect) {
                var $ele = opts.effect[key];
                var value = $el.attr(key);
                if($.isArray($ele)){
                    for (var i = 0; i < $ele.length; i++) {
                        if($ele[i].is(".ui-select")){
                            $ele[i].ui_select().val(value);
                            $ele[i].change();
                        } else {
                            $ele[i].val(value);
                            $ele[i].change();
                ...
                } else if($ele.is(".ui-select")){
                    $ele.ui_select().val(value);
                    $ele.change();
                } else {
                    $ele.val(value);
                    $ele.change();
        ...
        clear : function () {
            ...
            $ele.val(this.updater("")).trigger("change");
            for ( var key in opts.effect) {
                var $ele = opts.effect[key];
                if($.isArray($ele)){
                    for (var i = 0; i < $ele.length; i++) {

...
(function($){
	$.fn.sidebar = function(data,callfunc){
		
		//创建DOM
		var 
		quickHTML = '<div class="quick_links_panel"><div id="quick_links" class="quick_links"><a href="#top" class="return_top"><i class="top"></i><span>返回</span></a><a href="#" class="my_qlinks"><i class="setting"></i><span>指标</span></a></div><div class="quick_toggle"><a href="javascript:;" class="toggle" title="展开/收起">×</a></div></div><div id="quick_links_pop" class="quick_links_pop hide"></div>',
		...
		if(data){
			for(i=0;i<data.length;i++){
				bardata+='<li><a href="#indiv_'+data[i].id+'" id="'+data[i].id+'">'+data[i].name+'</a></li>';
			}
		}
	    ...
	    panheight = $(window).height()*0.7;
		if(panheight<300){
	        panheight = 300;
	    }

	    popTmpl = '<div class="title" title="'+quickDataFns.my_qlinks.title+'"><h3><i></i>'+quickDataFns.my_qlinks.title+'</h3></div>' +
			'<div class="pop_panel" style="max-height:'+panheight+'px">'+quickDataFns.my_qlinks.content+'</div><div class="arrow"><i></i></div><div class="fix_bg"></div>'
		...
		showQuickPop = function(type){
			...
			if(type !== prevPopType){
				...
				quickPop.html(popTmpl);
				$('#quick_links_pop li').bind('click',function(e){
					callfunc(e);
				})

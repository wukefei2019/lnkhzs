...
<head>
	...
	<script language="javascript">
		...
		window.onload = function()
		{
			...
			//fying 2015-06-10 在此处添加‘已选人员、部门’初始化逻辑
			actorMap = new Map();
			var selectedActor = parent.getSelectedActor();
			if(selectedActor != '')
			{
				var actorArray = selectedActor.split(';');//U_Demo_modetype-系统管理员;D_*********_modetype-部门名称;
				for(var i=0;i<actorArray.length;i++)
				{
					var actorString = actorArray[i];
					if(actorString != '')
					{
						var type = actorString.substring(0, 1);
						var id = "";
						var name = "";
						var mode = "";
						//部门与人员的拼接串不同    【人员 ：U_*******_2-Demo-系统管理员] 【部门：D_********_2-部门名称】
						if(type == 'D')
						{
						   id = actorString.split('-')[0];
						   mode = id.substring(id.length-1, id.length);
						   id = id.substring(0, id.lastIndexOf('_'));
						   name = actorString.split('-')[1];
						}else{
						   id = actorString.substring(0, actorString.lastIndexOf("-"));
						   mode = id.substring(id.length-1, id.length);
						   id = id.substring(0, id.lastIndexOf('_'));
						   name = actorString.substring(actorString.lastIndexOf("-")+1,actorString.length);
						}
						document.getElementById('commonResultArea').contentWindow.initOneResult(id, name, actorString.substring(0, 1),mode);
						actorMap.put(id, name+":"+mode);
					}
				}
				
				
				document.getElementById('commonResultArea').contentWindow.setTimeValue(parent.limittimes,parent.dealtimes);
			}
		...
		function addActor(id,name,mode)
		{
			actorMap.put(id, name+':'+mode);
		}
		
		function  updateActor(id,mode)
		{
			var name = actorMap.get(id);
			name = name.split(":")[0];
			actorMap.put(id, name+':'+mode);
		}
		...
		{
			var data = document.getElementById('commonResultArea').contentWindow.getTimeValue();
			return data;
		}
...
<body>
	<div class="content">
		...
		<iframe src="${ctx}/common/tools/orgtree/assignTreeResultArea.jsp?isRadio=<%=isRadio%>&utType=<%=utType%>&typeMark=<%=typeMark%>" frameborder="0" id="commonResultArea" name="commonResultArea" height="100px" width="100%"></iframe>
		<%
		if("".equals(useMode))
		{
		%>

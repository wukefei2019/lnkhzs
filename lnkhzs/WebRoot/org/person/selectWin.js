...
var all_selection_my=[];

function table_check(row) {
	var all_selection = $("#table1").bootstrapTable("getAllSelections");
	for (var i = 0; i < all_selection_my.length; i++) {
		if(all_selection_my[i].PID==row.PID){
			all_selection_my.splice(i, 1);
			break;
		}
	}
	if(row.state){
		all_selection_my.push(row);
	}
	var htmls = [];
	for (var i = 0; i < all_selection_my.length; i++) {
		var row = all_selection_my[i];

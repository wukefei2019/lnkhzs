	...
	function getPieJsonById(myChart, param) {	
		...
		$.post(url).done(function(data) {
			...
			for (var i = 0; i < data.length; i++) { 
				var value = data[i].rowHashMap.移动总量;
				var name = data[i].rowHashMap.CITY;
				var line = data[i].rowHashMap.移动解决率;
				if(name=='合计')continue;
				if(name=='全省')continue;
				nameTemp[j] = name;
				valueTemp[j] = value;
	...
	<body style="width: 100%; height: 100%;background-size:100% 100%;">
		...
		<div>
			<label for="inputCala" class="radioS selected" ><span>时间</span></label> 
 			<input  type="text" radioS="inputCala" autocomplete="off" class="textInput inputCala" id="calendar"

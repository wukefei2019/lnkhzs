...
var sqlNamec="";
	...
	function getPieJsonById(myChart, param) {	
		var url = $ctx + "/trace/traceReport/comLinePie.action?sqlName=SQL_TRACE_MARKETING_CITY.query&"+ encodeURI(param);
		$.post(url).done(function(data) {
			...
			for (var i = 0; i < data.length; i++) { 
				var value = data[i].rowHashMap.营销活动总量;
				var name = data[i].rowHashMap.CITY;
				var line = data[i].rowHashMap.营销活动解决率;
				if(name=='合计')continue;
				nameTemp[j] = name;
				valueTemp[j] = value;
				lineTemp[j] = line;
			...
			option_pie.yAxis[0].data = lineTemp;
			option_pie.xAxis[0].data = nameTemp;
			option_pie.series[0].data = lineTemp;

		...
		$(document).ready(function() {
			...
			$.post(url).done(function(data) {
				...
				for (var i = 0; i < mynum; i++) {
					...
					var colorSplit = i % 7;
					var url = $ctx + "/lnkhzs/propage/indAjax.jsp?id=" + id + "&sql=" + sql + "&type=" + type + "&colorSplit=" + colorSplit;
					html += '<div class ="divcontent" ><div class="titleDiv"><span class="paddingL5">' + name + '</span></div>'
						+ '<div onclick="contentDivbtn(this)" class="contentDiv" id="' + id + '"><div id="zhezhao' + id + '" style="z-index:99;width: 100%;height: 100%;position: absolute;left: 0;top: 0;background-color: rgba(0,0,0,0);display:table;text-align:center;"></div><iframe src=' + encodeURI(url) + ' frameborder="0"  scrolling="no"  class="width100p" style="height: 100%;"></iframe></div></div>';
				...
				/* $(".contentDiv").on('click', function(e) {
				...
				for (var i = 0; i < mynum; i++) {
					...
					if (vieworder != null && vieworder > 0) {
						...
						$("#" + id).append('<div style="width: 100%;height: 100%;position: absolute;left: 0;top: 0;background-color: rgba(0,0,0,0.4);display:table;text-align:center;"><div class="addDiv"><img src="../style/image/finish.png" class="add"/></div></div>');
					} else {
						$("#" + id).append('<div style="width: 100%;height: 100%;position: absolute;left: 0;top: 0;background-color: rgba(0,0,0,0.4);display:table;text-align:center;"><div class="addDiv"><img src="../style/image/loading_0.gif" class="add"/></div></div>');
					}
				}
		...
		window.onscroll = function() {
			if (getScrollTop() + getClientHeight() == getScrollHeight()) {
				...
				for (var i = mynum; i < maxlength; i++) {
					...
					var url = $ctx + "/lnkhzs/propage/indAjax.jsp?id=" + id + "&sql=" + sql + "&type=" + type + "&colorSplit=" + colorSplit;
					html += '<div class ="divcontent" ><div class="titleDiv"><span class="paddingL5">' + name + '</span></div>'
						+ '<div onclick="contentDivbtn(this)" class="contentDiv" id="' + id + '">'
						+ '<div id="zhezhao' + id + '" style="z-index:99;width: 100%;height: 100%;position: absolute;left: 0;top: 0;background-color: rgba(0,0,0,0);display:table;text-align:center;"></div>'
						/* +'<div style="width: 100%;height: 100%;position: absolute;left: 0;top: 0;background-color: rgba(0,0,0,0.4);display:table;text-align:center;"><div class="addDiv"><img src="../style/image/loading_0.gif" class="add"/></div></div>' */

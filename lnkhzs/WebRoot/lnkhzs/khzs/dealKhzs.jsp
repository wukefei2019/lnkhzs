...
/*     		savepath:'eoms_workSheet/WF4_EL_UVS_TSK/2021-01-18' */
		...
		$("#folwuser").on("click", function(event) {
	    ...
	    if(volid.length!=0){
	    	$.post($ctx + "/khzs/flow/khzsTFlowAction/ajaxGetKhzsTMain.action?khzsTMain.pid="+volid).done(function(result) {
				...
				$("#folwremark2").val(result["solution"]);
				...
				if(nextloginname==nowLoginUserFullName){
					 $("#approve").show();
				}
    ...
    function fn_save() {
        ...
        var url =encodeURI($ctx + "/khzs/flow/khzsTFlowAction/doAction.action?khzsTFlow.remark2="+$("#folwremark2").val()
        		...
        		+"&selectUser.loginName="+$("#folwuserid").val()+"&khzsTMain.solution="+$("#folwremark2").val()
    ...
    <body>
        <div class="Ct Dialog contract form-page" >
        		<div id="approve" class='panel panel-primary' style="display: none">
	                <div class='panel-heading'>
	                    <h3 class='panel-title'>
	                    	审批信息
	                    	<a data-toggle="collapse" href="#approvePart" style="text-decoration: none" class="glyphicon glyphicon-my-minus"></a>
	                    	<a class='btn floatRight10 transfer'>转派</a>
	                    	<a class='btn floatRight10 handOut'>提交审核</a>
	                    	<a class='btn floatRight10 hangup' style='display: none'>挂起</a>
						</h3> 
	                </div>
	                <div id="approvePart" class="collapse in">
		                <input id='folwuserid' type='hidden'/>
		                <input id='folwdeptname' type='hidden'/>
		                <div class="form-table">
			                <div class='col-xs-3'>
								<label>选择负责人：</label>
							</div>
							<div class='col-xs-9'>
								<input id='folwuser' type='text' style='width:100%' class='form-control' readonly='true'/>
							</div>
							<div class='col-xs-3'>
								<label>审批意见：</label>
							</div>
							...
							<div class='col-xs-9'>
								<input id="folwremark2" type='text' style="width:100%" class='form-control' value="${khzsTMain.solution}"/>
							</div>
						</div>
	                </div>
                   ...
                   <form id='form' name='form'target="frm" class="collapse in">
					...
					<div class="form-table">
						...
						<div class='ygts col-xs-9'>
							<input id="source" type='text' style="width:100%" class='form-control'	name='khzsTMain.source' value='${khzsTMain.source}' readonly="true"/>
						</div>
						<div class='col-xs-3'>
							<label>当前处理方案：</label>
						</div>
						<div class='col-xs-9'>
							<textarea id="solution2" class='txtarea1' name='khzsTMain.solution2' >${khzsTMain.solution2}</textarea>
						</div>
						<div class='col-xs-3'>
							<label>解决方案：</label>
						</div>
						<div class='col-xs-9'>
							<textarea id="solution" class='txtarea1' >${khzsTMain.solution}</textarea>
						</div>
						<div class='ygts col-xs-3'>
							<label>待解决问题：</label>
						</div>
						<div class='ygts col-xs-9'>
							<textarea id="question" class='txtarea1'	name='khzsTMain.question' >${khzsTMain.question}</textarea>
						</div>
						<div class='col-xs-3'>
							<label>备注：</label>
						</div>

...
<script type='text/javascript'>
	...
	<%
    	...
    	if(fileid==null){
    		fileid=UUIDGenerator.getUUIDoffSpace();
    	}
    %>
...
/*     		savepath:'eoms_workSheet/WF4_EL_UVS_TSK/2021-01-18' */
		...
		//保存
		$(".handOutSave").on("click", function(event) {
			$("#sworkstatus").val("已保存");
			fn_save(0);
		});	
	...
	function getuserdept(){
		var url = $ctx + "/tsgd/swork/getUserDept.action";
		$.post(url).done(function(data) {
			$("#resendpeo").val(data.username);
			$("#resenddep").val(data.deptname);
			$("#resendnum").val(data.mobile);
	...
	function ShowselMeu(index){
		var arr = [["--选择类别--"],["以案促改","回头看"],["集团服务督办单"],
		           ["全网投诉工作情况通报","5G投诉情况通报","全网携号转网投诉情况通报","“阳光行动”工作情况通报","集团客户满意度和产品体验评测情况的通报",
				"互联网电视及家庭产品质量评测情况的通报","邮件：工信部申诉受理中心重点调查案件","邮件：携号转网代客申诉情况分析","服务管理处微信群","质量标准处微信群",
				"质量评测处微信群","客户服务协调中心微信群","10080重点核查单","辽宁省通信管理局：协助用户反映问题"],["溯源回头看"],["其他"]];
		//var index = document.getElementById("selID").selectedIndex;
		var subNode = document.getElementById("subID");
		var i = arr[index];		 		
		subNode.options.length = 0;   //清空之前选项
		...
		<div id="formTable" style="margin-top: 10px;"
			...
			<div class='panel-heading'>
				<h3 class='panel-title'>
					...
					<a class='btn floatRight10 handOut' style='display: inline;'>发起</a>
			...
			<form id='form' name='form' target="frm" class="collapse in">
				...
				<input id="pid" type="hidden" name="sWork.pid" value='${sWork.pid }' />
				<input id="sworkstatus" type="hidden" name="sWork.status" value='${sWork.status }' />

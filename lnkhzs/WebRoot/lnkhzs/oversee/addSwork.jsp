...
/* 	function getNumber(){
		alert("aaaa");
			var url = encodeURI($ctx + "/tsgd/swork/getNumber.action");
			$.post(url, $("form").serialize()).done(function(serial_number) {
//               控制台输出一下servlet传过来的值
...
//                 获取编号的input标签，并赋值传过来的值
	 ...
	 */
	 ...
	 function fn_save(optinon) {
		...
		||$("#selID").val() == ""||$("#subID").val() == ""||$("#subID").val() == null||$("#selID").val() == null||$("#selID").val() == "--选择事项--"||$("#selID").val() == "--选择类别--"
	...
	function channgeoption(obj) {
		...
		/* var url = $ctx + "/tsgd/swork/getTopUserByDep.action?depid=" + $(obj).find("option:selected").attr("myid");

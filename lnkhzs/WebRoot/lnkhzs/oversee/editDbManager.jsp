...
<script type='text/javascript'>
	...
	function getDetail(){
		var id = "${param.id}";
		if (id.length != 0) {
			$.post($ctx + "/tsgd/resendmanager/getManagerDetail.action?dbmanager.id=" + id).done(function(result) {
				$("#id").val(result.id);
	...
	var setDutyUser = function(rows,fn){
		$("[name='dbmanager.administratorname']").val($.map(rows,function(t,i){return t.FULLNAME}).join(","));
    
    	$("#administratorid").val($.map(rows,function(t,i){return t.LOGINNAME}).join(","));
    }

	function fn_save(optinon) {
		var url = encodeURI($ctx + "/tsgd/resendmanager/saveManager.action");
	...
	<div class="Ct Dialog contract form-page">
		<div id="formTable" class='panel panel-primary'>
			...
			<form id='form' name='form' target="frm" class="collapse in">
				<input id="id" type="hidden" name="dbmanager.id"
					value="${dbmanager.id }" />
				<input id="administratorid" type="hidden" name="dbmanager.administratorid"
					value="${dbmanager.administratorid }" />
				...
				<div class="row" style="margin-left: 0px;margin-right: 0px;">
					...
					<div class='col-xs-9' id="showUsers">
							...
							<input type='text' id="administratorname" class='inputText1 width160' name='dbmanager.administratorname'
								value='${dbmanager.administratorname}' style="width:70%;" /><span
					...
					<div class='col-xs-9'>
						<%-- <input id="cityname" type='text' style="width: 100%"
							...
							<select id="cityname" 
								class='form-control' name='dbmanager.cityid'></select>
					</div>
					</div>
			</form>
		</div>

...
var submit = function(value,adminLoginname){
		var array = $('#table1').bootstrapTable('getAllSelections');
		var data=[];
		for(var i=0;i<array.length;i++){
			data.push(array[i].PID);
		}
		$.post($ctx + '/trace/flow/doAction.action',{data:JSON.stringify( array ),actionStr:'提交',dealtime:value,loginName:adminLoginname}).done(function(result){
			if (result == 'success') {
				alert("操作成功");
...
var submitAll = function(data,adminLoginname){
		var rQueryName= $("#table0").attr("sqlname");
		var url = $ctx + '/trace/flow/selectAll.action?&rQueryName='+rQueryName;
	 	$(".serachdiv:first").find("input,select").each(function(){
	 		if($(this).val()!=null){
	 			url+="&params."+ $(this).attr("name")+"="+ $(this).val();
	 		}
	 	});
		$.post(encodeURI(url),{data:data,loginName:adminLoginname}).done(function(result){
			if (result == 'success') {
				alert("操作成功");
				$(document).find(".btn-link.icon-refresh2").click();
			} else {

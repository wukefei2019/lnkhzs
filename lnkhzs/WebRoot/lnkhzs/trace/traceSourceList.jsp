...
<meta charset="UTF-8">
<title>溯源问题清单</title>
<script type="text/javascript" src="${ctx }/lnkhzs/trace/traceSourceList.js"></script>
	...
	<div class="Ct Dialog">
		...
		<dg:ajaxgrid id="table0" sqlname="SQL_ZL_TRACE_SOURCE_LIST.query" inmap="${inmap }">
			<dg:lefttoolbar>
	            ...
	            <button class="btn-link iconfont fontsize14 icon-upload1" onclick="$.bs.modal.open('${ctx}/lnkhzs/trace/import.jsp',width=500,height=300)">导入</button>
 	            <button class="btn-link iconfont fontsize14 icon-daochu" onclick="exportVal()">导出</button> 
			</dg:lefttoolbar>
			<dg:condition>
			...
			<!-- <button class="btn floatRight10" style="float:right" onclick="deleteAll()">删除全部</button>  -->
			<button class="btn floatRight10" style="float:right" onclick="allSubmit()">全部提交</button> 
			<button class="btn floatRight10" style="float:right" onclick="singleSubmit()">选中提交</button> 
			<!-- <button class="btn floatRight10" style="float:right" onclick="remove()">选中删除</button> --> 
				 ...
				 class="table table-hover text_align_center table-no-bordered"  data-show-columns='false' data-row-attributes="rowStyle">
					<thead>
						<tr>
							...
							<th data-field="SERIALNO" data-formatter="fn_checkbox_disable" data-sortable="true">编号</th>
							<th data-field="LINK_PROGRESS" data-formatter="fn_show" data-sortable="true">环节进展</th>

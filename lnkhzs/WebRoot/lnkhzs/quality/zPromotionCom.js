...
$(document).ready(function(){
	...
	var tablenameSLQD = getTablenameSLQD();//受理渠道
	var tablenameYHXJ = getTablenameYHXJ();//用户星级
	var tablename="";
	
	//获取列表
    $.post($ctx + "/tsgd/tsgd/ajaxGetSearchTree.action?tableName=" + tablenameSLQD, {}).done(function(entities) {
    	$("#SLQD").append("<option value=''></option>"); 
    	for(var i = 0;i<entities.length;i++){
    		var en = entities[i].name;
    		if(en!=""&&en!=undefined){
    			$("#SLQD").append("<option value='"+en+"'>"+en+"</option>"); 
    		}
    	}
    });
    
    $.post($ctx + "/tsgd/tsgd/ajaxGetSearchTree.action?tableName=" + tablenameYHXJ, {}).done(function(entities) {
    	$("#YHXJ").append("<option value=''></option>"); 
    	for(var i = 0;i<entities.length;i++){
    		var en = entities[i].name;
    		if(en!=""&&en!=undefined){
    			$("#YHXJ").append("<option value='"+en+"'>"+en+"</option>"); 
    		}
    	}
    });
...
function onTreeClick(event, treeId, treeNode, clickFlag) {
   	var treeObj = $.fn.zTree.getZTreeObj("tree1s");    
    // 获取当前勾选数据的值

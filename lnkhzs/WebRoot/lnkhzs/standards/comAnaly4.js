...
function getParams() {
	...
	var params = "&area_name=" + cityName + "&time=" + dateVal + "&type=集团专线";
	return params;
}
...
function initCharts() {
	...
	var url = $ctx + "/standards/fwzl/ajaxGetPiesJsonGroup.action?menuId=A&unit=A" + encodeURI(urlParam);
...
//
			...
			echertPie.on('click', function (param){
				console.log(param);
				var childUrl=$ctx + "/lnkhzs/standards/comLine4.jsp?name="+param.data.names+"&params="+urlParam;
...
function addInd(){
	var urlParam = getParams();
	openwindow($ctx + "/lnkhzs/standards/indAddGroup.jsp?menuId=B"+encodeURI(urlParam));
}
